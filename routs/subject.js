...
router.get('/',(req,res)=>{
  ...
  model.Subject.findAll({order:[['id','asc']]}).then(subjects => {
    ...
    .then(newDataSubject=>{
      res.render('subject',{data:newDataSubject,title:`Subjects`});
    })
 })
  .catch(err=>{
    res.send(err);
...
router.get('/:id/enrollstudents', (req,res) => {
  model.StudentSubject.findAll({where:{SubjectId:req.params.id},attributes: [['id','id'],['StudentId','StudentId'],['SubjectId','SubjectId'],['score','score']]}).then(subjects => {
    ...
    Promise.all(promSubject).then(student => { //memangil semua promise
      model.Subject.findById(req.params.id).then(subject => {
        res.render('subject_enrollstudent', {student:student,subject:subject,title:'Enroll Student'})
      })
    }).catch(err => {
      res.send(err)
    })
    ...
    if(studentSubject){
      studentSubject.getStudent().then(student => {
        studentSubject.getSubject().then(subject => {
          ...
          res.render('subject_givescore', {data:studentSubject,title:'Give Score'})
        })
      })
    }
  }).catch(err => {
...
router.get('/add', (req,res) => {
  res.render('subject_add', {msg:'',title:'Add Subject'})
})
