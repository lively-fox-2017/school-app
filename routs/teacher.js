...
router.get('/', (req,res) => {
	model.Teacher.findAll({order:[['id','asc']]}).then(teachers => {
		...
		Promise.all(arr_prom).then(subjects =>{
			...
			res.render('teacher', {data:teachers,title:'Teachers',role:req.session.user})
		})
	})
})
router.get('/add', (req,res) => {
	model.Subject.findAll().then(subjects => {
		res.render('teacher_add', {subject:subjects,msg:'',title:'Add Teachers',role:req.session.user,role:req.session.user})
	})
})
...
router.get('/edit/:id', (req,res) => {
	Promise.all([
		...
		model.Subject.findById(resource[1].SubjectId).then(subject => {
			...
			res.render('teacher_edit', {data:resource[1],subject:resource[0],msg:'',title:'Edit Teacher',role:req.session.user})
		})
	}).catch(err => {
		res.send(err)
	})
