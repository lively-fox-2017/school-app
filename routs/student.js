...
router.get('/', (req,res) => {
	model.Student.findAll({order:[['first_name','asc']]}).then(students => {
		res.render('student', {data:students,title:'Student'})
	}).catch(err => {
		res.send(err)
	})
})
router.get('/add', (req,res) => {
	res.render('student_add', {msg:'',title:'Add Student'})
})
router.post('/add', (req,res) => {
	model.Student.create(req.body).then(add => {
		...
		model.Student.findAll().then(students => {
			res.render('student_add', {data:students,msg:err.errors[0].message,title:'Add Student'})
		})
	})
})
router.get('/edit/:id', (req,res) => {
	model.Student.findById(req.params.id).then(student => {
		res.render('student_edit', {data:student,msg:'',title:'Edit Student'})
	}).catch(err => {
		res.send(err)
	})
})
router.post('/edit/:id', (req,res) => {
	model.Student.update(req.body,{where:{id:req.params.id},validate:false}).then(update => {
		...
		model.Student.findById(req.params.id).then(student => {
			res.render('student_edit', {data:student,msg:err.errors[0].message,title:'Edit Student'})
		})
	})
})
...
router.get('/:id/addsubjects', (req,res) => {
	Promise.all([
		...
		res.render('student_addsubject',{student:resource[0],subject:resource[1],title:'Add Subject'})
	}).catch(err => {
		res.send(err)
	})
})
